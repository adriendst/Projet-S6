...
const DetailPage = () => {
    ...
    useEffect(() => {
        if (param.gameid !== undefined) {
            if (idGamesUser.includes(Number(param.gameid))) {
                setIsAlreadyInUserGame(false)
            }
            else{
                setIsAlreadyInUserGame(true)
            }
        }
    }, [idGamesUser]);
    ...
    const items: TabsProps['items'] = [
        {
            key: '1',
            label: `Windows`,
            children: data?.pc_requirements && Object.keys(data.pc_requirements).map((keyName, i) => <p
                dangerouslySetInnerHTML={{__html: data?.pc_requirements[keyName]}}/>),
        },
        {
            key: '2',
            label: `macOs`,
            children: data?.mac_requirements && Object.keys(data.mac_requirements).map((keyName, i) => <p
                dangerouslySetInnerHTML={{__html: data?.mac_requirements[keyName]}}/>),
        },
        {
            key: '3',
            label: `Linux`,
            children: data?.linux_requirements && Object.keys(data.linux_requirements).map((keyName, i) => <p
                dangerouslySetInnerHTML={{__html: data?.linux_requirements[keyName]}}/>),
        },
    ];
        ...
        <div>
            ...
            <Row align="middle">
                ...
                <Col xs={24} sm={24} md={24} lg={8} xl={9}>
                    ...
                    >
                        ...
                        <Descriptions.Item
                            label="">{data?.steamspy_tags && Object.keys(data.steamspy_tags).map((keyName, i) => <Tag
                            key={i}>{data?.steamspy_tags[keyName]}</Tag>)}</Descriptions.Item>
                    </Descriptions>
                </Col>
            </Row>
            ...
            <Row>
                <Col xs={{span: 24, order: 2}} sm={{span: 24, order: 2}} md={{span: 24, order: 2}}
                     lg={{span: 14, order: 1}} xl={{span: 15, order: 1}}>
                ...
                <Col xs={{span: 24, order: 1}} sm={{span: 24, order: 1}} md={{span: 24, order: 1}}
                     lg={{span: 8, order: 2}} xl={{span: 9, order: 2}}>
                    <Descriptions title="Other Information" column={1}>
                        <Descriptions.Item
                            label="Genres">{data?.genres && Object.keys(data.genres).map((keyName, i) => data?.genres[keyName] + ' ')}</Descriptions.Item>
                        <Descriptions.Item
                            label="Categories">{data?.categories && Object.keys(data.categories).map((keyName, i) => data?.categories[keyName] + ' | ')}</Descriptions.Item>
                        <Descriptions.Item
                            label="Platforms">{data?.platforms && Object.keys(data.platforms).map((keyName, i) => <Tag
                            key={i}>{data?.platforms[keyName]}</Tag>)}</Descriptions.Item>
                    </Descriptions>
